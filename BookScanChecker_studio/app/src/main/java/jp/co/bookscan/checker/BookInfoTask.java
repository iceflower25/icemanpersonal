...
class BookInfoTask extends AsyncTask<String, Void, Integer> {
    ...
    private static String numberOfBooks = null;
   ...
   public static class BookInfoDialog extends DialogFragment {
		public Dialog onCreateDialog(Bundle state) {
	        ...
	        if (pc_name == null) {
                ...
                ((TextView)bookinfoView.findViewById(R.id.text_bookcounter)).setText(numberOfBooks);
    ...
    private void parseInfo(String data) throws JSONException {
        ...
        for (int i = 0; i < l; i++) {
            ...
            numberOfPages = obj.optString("NumberOfPages", numberOfPages);
            try {
                pages = Integer.parseInt(numberOfPages);
                bookNum = pages > oneBook ? 1 + (pages - oneBook) / extraBook + ((pages - oneBook) % extraBook > 0 ? 1 : 0) : 1;
                numberOfBooks = Integer.toString(bookNum) + activity.getResources().getString(R.string.bookcounterunit);
            } catch(Exception e) {
                numberOfBooks = activity.getResources().getString(R.string.unknownbook);
            }
